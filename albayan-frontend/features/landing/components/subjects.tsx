      ...
      <div className="mx-auto w-full max-w-6xl px-4 py-20 sm:px-6">
        ...
        <StaggerGroup className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-5" step={120}>
          {SUBJECTS.map((subject) => (
            ...
            >
              <span className="mb-4 text-4xl transition-transform duration-300 group-hover:-rotate-3 group-hover:scale-110">
                {subject.icon}
              </span>
              <h3 className="font-bold">{subject.name}</h3>

              <div className="mt-3 space-y-1.5 text-xs text-muted-foreground">
                <p>📚 {subject.units} وحدات</p>
                <p>🎯 {subject.lessons} درسًا</p>
                <p>🧩 {subject.questions} سؤالًا</p>
              </div>
